{ config, pkgs, lib, ... }: with lib; let
  ...
  dynamicWallpaperScriptPart = name: wpGroups: let
    dir = "${home}/Pictures/wallpapers/Apple/${name}";
  ...
  setWallpaperScript = let
    type =
      if cfg.type == "mojave_dynamic" then dynamicWallpaperScriptPart "mojave_dynamic" [
      ...
      else if cfg.type == "catalina_dynamic" then dynamicWallpaperScriptPart "catalina_dynamic" [
        { hours = [ 0 1 2 ]; wpNum = 1; }
        { hours = [ 3 4 5 ]; wpNum = 2; }
        { hours = [ 6 7 8 ]; wpNum = 3; }
...
in
{
  options = {
    wallpaper = {
      type = mkOption {
        type = types.enum [ "mojave_dynamic" "catalina_dynamic" "random_bg" ];
